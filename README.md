...
├── pubspec.yaml
├── lib/
│   ├── main.dart             # Entry point, langsung buka SplashScreen
│   ├── splash_screen.dart    # Animasi pembuka "COOLER CONTROLLER"
│   ├── auth_screen.dart      # Login & Daftar/Klaim cooler baru (BLE)
│   └── controller_page.dart  # Halaman kontrol (kontrol via WiFi/MQTT atau BLE)
└── firmware/
    └── firmware.ino     # Source firmware ESP32-C3 Super Mini
```

## 👥 Multi-cooler / Multi-user (akun per unit)
Kalau firmware ini dipakai lebih dari 1 unit (misal kamu + beberapa teman
punya cooler masing-masing), setiap unit ESP32 wajib **diklaim** dulu
pakai username & password lewat app, supaya tidak saling nyala/tabrakan
saat pakai broker MQTT publik yang sama:

1. Install app, buka tab **"Tambah Cooler Baru"** di halaman Login.
2. Buat username & password bebas (khusus punya kamu sendiri).
3. Nyalakan Bluetooth HP, tempelkan HP dekat unit ESP32 yang mau diklaim
   → app akan scan & tampilkan daftar unit BLE bernama `ESP32-Cooler`.
4. Pilih unit itu → app kirim data akun ke ESP32 lewat Bluetooth →
   ESP32 menyimpan akun itu **permanen** (tidak hilang walau mati listrik).
5. Setelah diklaim, unit itu HANYA merespon perintah dari akun yang
   mengklaimnya — baik lewat WiFi/MQTT maupun BLE.
6. Teman-temanmu ulangi langkah 1-4 pakai username masing-masing untuk
   unit cooler mereka sendiri.

Kalau unit ESP32 mau dipindah tangan / akunnya lupa, satu-satunya cara
reset adalah **erase & flash ulang firmware** (pakai tombol Erase di app
flasher ESP32_Flasher) — ini menghapus akun yang tersimpan di flash.
...
### Board decoy → ESP32-C3 (pilih voltase 5V/9V/12V)
Pakai output **A** di board decoy (yang deket IC). Solder kabel dari pad **1** dan **2** ke ESP32 (JANGAN solder pad 3 & 4 — tidak dipakai):
...
Kalau kedua GPIO 6 & 7 tidak aktif (default) → board tetap di 5V (tidak perlu solder apapun untuk mode 5V).
...
## 🛠️ Panel Developer (dashboard admin)
App ini punya halaman tersembunyi khusus developer untuk memantau:
- Berapa cooler yang sedang **online**
- Berapa **HP** yang pernah install & buka app ini
- Daftar **username** yang terdaftar (password **tidak pernah** ditampilkan
  di mana pun — cuma tersimpan sebagai hash satu-arah, bukan teks asli)
- Tombol **hapus akun** pengguna kapan saja

Halaman ini **tidak muncul di menu manapun** untuk pengguna biasa. Satu-satunya
jalan masuk: tahan lama (long-press) logo ❄️ di halaman Login.

### Setup Firebase — SUDAH SELESAI ✅
Project Firebase `fan-cooler-app` sudah dibuat dan sudah tersambung ke
kode ini:
- `lib/firebase_options.dart` sudah terisi API key asli (bukan placeholder).
- `firebase/google-services.json` sudah ada di repo, `codemagic.yaml`
  otomatis memasangnya ke tempat yang benar tiap build.
- Akun developer: username **`admin12`**, password **sesuai yang kamu
  set saat "Add user"** di Firebase Console tadi.
- UID developer (`ljp6iPTaxnSvyJLkSGdw5oCMOi53`) sudah ditempel langsung
  di `firestore.rules`.

**Satu langkah terakhir yang WAJIB kamu lakukan manual di HP** (tidak
bisa diotomatiskan lewat kode karena ini aturan keamanan server, bukan
bagian dari APK):

1. Buka [console.firebase.google.com](https://console.firebase.google.com)
   → project **fan-cooler-app** → **Databases & Storage** → **Firestore
   Database** → tab **Rules**.
2. Hapus semua isi kotak rules yang ada, ganti dengan **seluruh isi**
   file `firestore.rules` yang ada di root zip ini (buka filenya, copy
   semua, paste di sana).
3. Tap **Publish**.

Tanpa langkah ini, Firestore masih pakai rules default (Production mode)
yang **menolak semua akses** — app akan error terus walau kodenya sudah
benar. Ini bukan bug, ini justru fitur keamanan (Firestore defaultnya
mengunci semua sampai kamu deploy rules sendiri).

Setelah rules ter-publish, tinggal build APK-nya lewat Codemagic seperti
biasa — semua tersambung otomatis.

Setelah semua langkah di atas selesai dan APK baru di-build, login developer
di app pakai username `admin12` & password yang kamu set di langkah 3.

**Login developer pakai Firebase Authentication sungguhan** (bukan sekadar
`if username == "admin12"` di kode) — supaya orang lain yang decompile APK
tidak bisa asal login sebagai developer.

## Cara kerja app
- 3 tombol preset: **5V / 9V / 12V** — ini satu-satunya cara pilih voltase (slider dihapus karena hardware cuma support 3 nilai tetap, bukan kontinu).
...
Tes dulu pakai multimeter (board decoy disambung ke charger USB-C, BELUM disambung ke ESP32): ukur resistansi pad 1 & 2 terhadap GND board. Kalau terbaca ada resistansi (bukan short 0Ω ke GND, dan bukan konek ke jalur output +), aman untuk disambung langsung ke GPIO ESP32.

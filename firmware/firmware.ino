...
//   - GPIO2 = strapping pin
//
// ===== SISTEM AKUN (username/password) =====
// Supaya beberapa unit cooler (misal punya kamu + punya teman-teman)
// bisa jalan bareng di broker MQTT publik yang sama TANPA saling
// nyala/tabrakan, setiap unit harus "diklaim" dulu pakai username +
// password lewat Bluetooth (BLE) sebelum bisa dikontrol.
// Setelah diklaim, unit ini HANYA mendengarkan topic MQTT unik milik
// username tersebut ("cooler/<username>/command"), dan HANYA
// menjalankan perintah yang username+password-nya cocok dengan yang
// tersimpan. Password yang dikirim dari app SUDAH dalam bentuk hash
// (SHA-256), bukan teks polos, supaya tidak gampang disadap dari lalu
// lintas MQTT publik.
// Sebelum diklaim, unit tetap bisa disolder & diuji manual (voltase
// default 5V), tapi TIDAK akan merespon perintah voltase dari app
// manapun sampai proses klaim (register) selesai.
// Untuk "lupa akun" / pindah tangan: hapus firmware & flash ulang
// (pakai tombol Erase di app flasher) supaya memori akun ikut hilang.
// ===================================================================
...
// Topic dibuat String (bukan const char*) karena isinya berubah-ubah
// sesuai username yang mengklaim unit ini.
String command_topic = "cooler/unclaimed/command";
String status_topic  = "cooler/unclaimed/status";
...
// ===== AKUN (disimpan permanen di flash) =====
String accountUser = "";
...
// ===== AKUN: simpan ke flash & bangun ulang topic MQTT =====
void rebuildTopics() {
  if (accountRegistered) {
    command_topic = "cooler/" + accountUser + "/command";
    status_topic  = "cooler/" + accountUser + "/status";
  } else {
    command_topic = "cooler/unclaimed/command";
    status_topic  = "cooler/unclaimed/status";
  }
}

void loadAccount() {
  preferences.begin("account", false);
  accountUser = preferences.getString("user", "");
  accountPassHash = preferences.getString("hash", "");
  preferences.end();
  accountRegistered = accountUser.length() > 0 && accountPassHash.length() > 0;
  rebuildTopics();
}

void saveAccount(String user, String hash) {
  preferences.begin("account", false);
  ...
  accountUser = user;
  accountPassHash = hash;
  accountRegistered = true;
  rebuildTopics();

  // Kalau sedang online, langsung pindah subscribe ke topic baru
  if (mqttClient.connected()) {
    mqttClient.subscribe(command_topic.c_str());
  }
}

// ===== PROSES PERINTAH MASUK (dipakai bareng oleh MQTT & BLE) =====
// Format pesan yang diterima ada 2 jenis:
// 1) Klaim akun baru   : {"action":"register","username":"...","passhash":"..."}
// 2) Perintah voltase  : {"voltage":9,"username":"...","passhash":"..."}
void processIncomingCommand(String msg) {
  ...
  if (strcmp(action, "register") == 0) {
    String user = String((const char*)(doc["username"] | ""));
    String hash = String((const char*)(doc["passhash"] | ""));
    user.trim();
    // Cuma boleh diklaim SEKALI. Kalau mau ganti pemilik, harus di-erase
    // & flash ulang dulu (lihat catatan di bagian atas file ini).
    if (user.length() > 0 && hash.length() > 0 && !accountRegistered) {
      saveAccount(user, hash);
    }
    return;
  }

  if (doc.containsKey("voltage")) {
    if (!accountRegistered) return; // belum ada pemilik -> abaikan semua perintah

    String user = String((const char*)(doc["username"] | ""));
    String hash = String((const char*)(doc["passhash"] | ""));
    if (user != accountUser || hash != accountPassHash) return; // username/password salah
...
void reconnectMQTTNonBlocking() {
  ...
  // Client ID disatukan dengan username (kalau sudah ada) supaya tiap
  // unit dapat client ID unik juga di broker publik.
  String clientId = "ESP32Cooler-" + (accountRegistered ? accountUser : String(random(0xffff), HEX));
...
void publishStatus() {
  ...
  doc["uptime"] = uptime;
  doc["registered"] = accountRegistered;
  doc["username"] = accountUser;
  String jsonStr;
...
// ===== API: SET AKUN (fallback lewat AP mode, selain lewat BLE) =====
void handleSetAccount() {
  if (server.hasArg("username") && server.hasArg("passhash")) {
    if (accountRegistered) {
      server.send(409, "text/plain", "Unit ini sudah diklaim akun lain");
      return;
    }
    saveAccount(server.arg("username"), server.arg("passhash"));
    server.send(200, "text/plain", "OK");
  } else {
    server.send(400, "text/plain", "Missing username or passhash");
  }
}
...
void setup() {
  ...
  server.on("/setaccount", handleSetAccount);
...
void loop() {
  // ===== Proses Perintah BLE (register atau voltase) =====
  if (bleCommand.length() > 0) {
    processIncomingCommand(bleCommand);
    bleCommand = "";
  }
